...
TEST(Workspace, InitCreatesPeerStructure) {
    ...
    auto ws = pear::storage::Workspace::init(td.dir);
...
TEST(Workspace, InitFailsInsideExistingWorkspace) {
    ...
    (void)pear::storage::Workspace::init(td.dir);
    ...
    EXPECT_THROW(
        (void)pear::storage::Workspace::init(sub),
        std::runtime_error
    );
}
...
TEST(Workspace, DiscoverFindsWorkspaceFromNestedDir) {
    ...
    auto ws = pear::storage::Workspace::discover(nested);
...
TEST(Workspace, DiscoverFailsWhenNoWorkspace) {
    ...
    EXPECT_THROW(
        (void)pear::storage::Workspace::discover(td.dir),
        std::runtime_error
    );
}
